...
export class JSONLParser {
  private readonly CLAUDE_PROJECTS_PATH = path.join(process.env['HOME'] || '~', '.claude', 'projects');
  ...
  private parseMessage(data: any, lineNumber: number): ClaudeCodeMessage | null {
    ...
    const message: ClaudeCodeMessage = {
      ...
      tokens: {
        input: data.tokens?.input || data.input_tokens || 0,
        output: data.tokens?.output || data.output_tokens || 0
      },
      processing_time_ms: data.processing_time_ms || data.duration_ms
    };

    if (data.tool_calls) {
      message.tool_calls = Array.isArray(data.tool_calls) ? data.tool_calls : [data.tool_calls];
    }

    if (data.tool_results) {
      message.tool_results = Array.isArray(data.tool_results) ? data.tool_results : [data.tool_results];
    }

    if (data.cache_stats || data.cache) {
      const cacheData = data.cache_stats || data.cache;
      message.cache_stats = {
  ...
  private buildSessionFromMessages(filePath: string, messages: ClaudeCodeMessage[]): ParsedSessionData {
    ...
    const databaseMessages: DatabaseMessage[] = sortedMessages.map((msg, index) => {
      ...
      if (msg.tool_calls) {
        msg.tool_calls.forEach((tool: any) => toolsUsed.add(tool.name));
      }
      ...
      return {
        ...
        tool_output: msg.tool_results?.[0] ? JSON.stringify(msg.tool_results[0].content) : undefined,
        timestamp: new Date(msg.timestamp),
        processing_time_ms: msg.processing_time_ms
      };
    });

    const totalCost = this.calculateCost(totalInputTokens, totalOutputTokens, this.detectModel(messages));
    ...
    const metrics: SessionMetrics = {
      ...
      cache_efficiency: totalCacheHits + totalCacheMisses > 0 
  ...
  async getAllSessionFiles(): Promise<string[]> {
    ...
    const projects = fs.readdirSync(this.CLAUDE_PROJECTS_PATH, { withFileTypes: true })
