...
function parseFilters(query: any, app: FastifyInstance): AnalyticsFilters {
  ...
  if (query.dateFrom) {
    ...
    if (isNaN(dateFrom.getTime())) {
      throw app.httpErrors.badRequest('Invalid dateFrom format. Use ISO 8601 format (YYYY-MM-DD or YYYY-MM-DDTHH:mm:ss.sssZ)');
    }
  ...
  if (query.dateTo) {
    ...
    if (isNaN(dateTo.getTime())) {
      throw app.httpErrors.badRequest('Invalid dateTo format. Use ISO 8601 format (YYYY-MM-DD or YYYY-MM-DDTHH:mm:ss.sssZ)');
    }
