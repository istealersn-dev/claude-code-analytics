...
import { DataRetentionManager, type RetentionConfig } from '../../database/retention.js';
...
export async function retentionRoutes(app: FastifyInstance) {
  ...
  app.get<{
    ...
    '/stats',
    async (request: FastifyRequest<{ Querystring: RetentionStatsQuery }>, reply: FastifyReply) => {
      const config: Partial<RetentionConfig> = {};

      // Parse optional config parameters
      if (request.query.sessionRetentionDays) {
        const days = parseInt(request.query.sessionRetentionDays, 10);
      ...
      if (request.query.messageRetentionDays) {
        const days = parseInt(request.query.messageRetentionDays, 10);
        if (Number.isNaN(days) || days < 1 || days > 3650) {
  ...
  app.get<{
    ...
    '/oldest',
    async (request: FastifyRequest<{ Querystring: OldestRecordsQuery }>, reply: FastifyReply) => {
  ...
  app.get('/validate', async (_request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.post<{
    ...
    '/clean',
    async (request: FastifyRequest<{ Body: CleanupRequestBody }>, reply: FastifyReply) => {
      const {
      ...
      if (sessionRetentionDays !== undefined) {
        const days = parseInt(sessionRetentionDays.toString(), 10);
        if (Number.isNaN(days) || days < 1 || days > 3650) {
          throw app.httpErrors.badRequest('sessionRetentionDays must be between 1 and 3650');
        }
        config.sessionRetentionDays = days;
      ...
      if (messageRetentionDays !== undefined) {
        const days = parseInt(messageRetentionDays.toString(), 10);
        if (Number.isNaN(days) || days < 1 || days > 3650) {
          throw app.httpErrors.badRequest('messageRetentionDays must be between 1 and 3650');
        }
        config.messageRetentionDays = days;
      ...
      if (metricsRetentionDays !== undefined) {
        const days = parseInt(metricsRetentionDays.toString(), 10);
        if (Number.isNaN(days) || days < 1 || days > 3650) {
      ...
      if (syncMetadataRetentionDays !== undefined) {
        const days = parseInt(syncMetadataRetentionDays.toString(), 10);
      ...
      if (batchSize !== undefined) {
        ...
        if (Number.isNaN(size) || size < 100 || size > 10000) {
  ...
  app.post<{
    ...
    '/schedule',
    async (request: FastifyRequest<{ Body: ScheduleRequestBody }>, reply: FastifyReply) => {
