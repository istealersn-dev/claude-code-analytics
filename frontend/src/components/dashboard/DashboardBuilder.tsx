...
import { type Layout as GridLayout, Responsive, WidthProvider } from 'react-grid-layout';
import {
  useDailyUsageTimeSeries,
  useDistributionData,
  useOverviewMetrics,
} from '../../hooks/useAnalytics';
import { BarChart, LineChart, PieChart as PieChartComponent } from '../charts/LazyCharts';
import type { DashboardTemplate, DashboardWidget } from './DashboardTemplates';
...
export const DashboardBuilder = memo(function DashboardBuilder({
  ...
  const renderWidget = useCallback(
    (widget: DashboardWidget) => {
      ...
      const sampleData =
        (dailyUsage?.sessions ?? []).slice(0, 30).map((item) => ({
          date: item.date,
      ...
      const renderWidgetContent = () => {
        switch (widget.type) {
          ...
          case 'pie-chart': {
            const pieData =
              (distributions?.modelUsage ?? []).slice(0, 5).map((item) => ({
                name: item.name,
                value: item.value,
              })) || [];
          ...
          case 'bar-chart': {
            const barData = sampleData.slice(0, 10).map((item) => ({
              name: item.date,
              value: item.value,
            }));
      ...
      return (
        // biome-ignore lint/a11y/useSemanticElements: This is a drag-and-drop container with react-grid-layout, not a traditional button
        <div
          key={widget.id}
          role="button"
        ...
        >
          ...
          {/* Widget Content */}
          <div className="p-2">{renderWidgetContent()}</div>
        </div>
      );
    },
    [
          ...
          {(() => {
            ...
            return (
              <div className="space-y-3">
                <div>
                  <label htmlFor="widget-title" className="block text-xs font-medium text-gray-300 mb-1">Title</label>
                  <input
                    id="widget-title"
                    type="text"
                    value={widget.title}
                ...
                <div>
                  <label htmlFor="widget-color" className="block text-xs font-medium text-gray-300 mb-1">Color</label>
                  <input
                    id="widget-color"
                    type="color"
                ...
                {widget.type !== 'metric-card' && (
                  <div>
                    <label htmlFor="widget-height" className="block text-xs font-medium text-gray-300 mb-1">
                      Height (px)
                    </label>
                    <input
                      id="widget-height"
          ...
          {template.widgets.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-gray-400">
              ...
              <p className="text-center mb-6 max-w-md">
                Add widgets from the side panel to create your custom analytics dashboard. Drag and
                resize widgets to arrange them as needed.
              </p>
              {isEditMode && (
                <button type="button"
